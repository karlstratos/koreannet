...
if __name__ == '__main__':
    ...
    parser.add_option("--jamos", dest="jamos_path", help="jamo (sub-character) information", metavar="FILE")
    ...
    parser.add_option("--jembedding", type="int", dest="jembedding_dims", default=10)
    ...
    parser.add_option("--onlyJamo", action="store_true", default=False)
    ...
    if not options.predictFlag:
        ...
        chars, c2i, words, w2i, pos, rels = utils.vocab(options.conll_train)

        char2jamos, jamos, j2i = utils.read_jamos(options.jamos_path)
        ...
        print
        #for (jamo,_) in sorted(jamos.items(), key=lambda x: int(x[1]), reverse=True):
        #    print jamo, 'count: ', jamos[jamo], 'index: ', j2i[jamo]
        #exit()

        #for char in char2jamos:
        #    print char,'\t',
        #    for jamo in char2jamos[char]:
        #        print jamo,
        #    print
        #exit()

        with open(os.path.join(options.output, options.params), 'w') as paramsfp:
            pickle.dump((char2jamos, jamos, j2i, chars, c2i, words, w2i, pos,
                         rels, options), paramsfp)
        print 'Finished collecting vocab'

        print 'Initializing blstm arc hybrid:'
        parser = ArcHybridLSTM(words, pos, rels, w2i, char2jamos, jamos, j2i,
                               chars, c2i, options)
        ...
        with open(options.params, 'r') as paramsfp:
            char2jamos, jamos, j2i, chars, c2i, words, w2i, pos, \
                rels, stored_opt = pickle.load(paramsfp)

        stored_opt.external_embedding = options.external_embedding

        parser = ArcHybridLSTM(words, pos, rels, w2i, char2jamos, jamos, j2i,
                               chars, c2i, stored_opt)
